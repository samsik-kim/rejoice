...
<s:form id="qnaListForm" name="qnaListForm" theme="simple" method="post">
	...
	<h1 class="fl pr10"><c:if test="${url eq 'searchDQnA.omp' || url eq 'removeDQnA.omp' || url eq 'sendDMail.omp'}">개발자 Q&A</c:if><c:if test="${url eq 'searchSCQnA.omp' || url eq 'removeQnA.omp' || url eq 'sendMail.omp'}">SC Q&A</c:if></h1>
	<c:if test="${url eq 'searchDQnA.omp' || url eq 'removeDQnA.omp' || url eq 'sendDMail.omp'}"><p>접수된 Q&A를 조회 / 답변할 수 있습니다.</p></c:if>
	<c:if test="${url eq 'searchSCQnA.omp' || url eq 'removeQnA.omp' || url eq 'sendMail.omp'}"><p>접수된 Q&A를 조회 / 답변할 수 있습니다.</p></c:if>
	...
	<table class="tabletype01">
		...
		<tr>
			<th>상태</th>
			<td class="align_td">
				<input type="radio" id="rb1" name="qna.prcStatCd" class="ml05" value="" <s:if test="qna.prcStatCd == ''">checked</s:if>/>전체  
				<input type="radio" id="rb2" name="qna.prcStatCd" class="ml05" value="CM000701" <s:if test="qna.prcStatCd == 'CM000701'">checked</s:if>/>대기 
				<input type="radio" id="rb3" name="qna.prcStatCd" class="ml05" value="CM000702" <s:if test="qna.prcStatCd == 'CM000702'">checked</s:if>/>답변완료
			</td>
			<td rowspan="5" class="text_c">
				<a class="btn" href="javascript:selectQnaList();"><strong>검색</strong></a>
				<a class="btn" href="javascript:init();"><span>검색초기화</span></a></td>
		</tr>
		<tr>
			<th>회원검색</th>
			<td>
		...
		<tr>
			<th>카테고리</th>
			<td>
				<s:select id="qnaCtgr01" name="qna.ctgrCd" theme="simple" list="categoryNameList" listKey="ctgrCd" listValue="ctgrNm" value="%{qna.ctgrCd}" headerKey="" headerValue="전체" style="width:104px;"/>
				<!-- 2차카테고리 -->
				<c:if test="${url eq 'searchSCQnA.omp' || url eq 'removeQnA.omp' || url eq 'sendMail.omp'}">
				...
				<s:else>
					<s:select id="qnaCtgr02" name="qna.qnaClsCd2" theme="simple" list="secondDepthCategoryList" listKey="ctgrCd" listValue="ctgrNm" value="%{qna.qnaClsCd2}" headerKey="" headerValue="전체" style="width:104px;"/>
				</s:else>
				</c:if>
			</td>
		</tr>
		<tr>
			...
			<td>
				<select id="keytype" name="qna.keytype" style="width: 104px;">
					<option value="both" <c:if test="${qna.keytype eq 'both' }">selected</c:if>>전체</option>
					<option value="title" <c:if test="${qna.keytype eq 'title' }">selected</c:if>>제목</option>
					<option value="dscr" <c:if test="${qna.keytype eq 'dscr' }">selected</c:if>>내용</option>
				</select> 
		...
		<tr>
			<th>작성일</th>
			<td class="align_td">
				<c:if test="${url eq 'searchDQnA.omp' || url eq 'removeDQnA.omp' || url eq 'sendDMail.omp'}"><label for="#">검색기간</label></c:if>
				<c:if test="${url eq 'searchSCQnA.omp' || url eq 'removeQnA.omp' || url eq 'sendMail.omp'}"><label for="#">검색기간</label></c:if>
				...
				<select id="dateType" name="qna.dateType" style="width:104px;">
                	<option value="" <c:if test="${qna.dateType eq ''}">selected</c:if>>전체</option>
					<option value="wait" <c:if test="${qna.dateType eq 'wait'}">selected</c:if>>대기</option>
					<option value="receipt" <c:if test="${qna.dateType eq 'receipt'}">selected</c:if>>답변완료</option>
                </select>
				
			 	<br />
				<a class="btn_s" href="javascript:search_date('today');"><strong>오늘</strong></a> 
				<a class="btn_s" href="javascript:search_date('7day');"><span>최근1주</span></a> 
				<a class="btn_s" href="javascript:search_date('14day');"><span>최근2주</span></a>
				<a class="btn_s" href="javascript:search_date('1month');"><span>최근1개월</span></a></td>
		</tr>
	</table>
