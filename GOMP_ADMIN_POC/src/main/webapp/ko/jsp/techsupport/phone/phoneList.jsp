...
<div id="contents_area">
	<!-- Title Area S -->
	<div id="ctitle_area">
		<p class="history">Home &gt; 개발지원센터 <strong>&gt;</strong> <span>단말정보</span></p>
		<h3><img src="<c:url value='/${ThreadSession.serviceLocale.language}/images/uc/uc_title04.gif' />" alt="단말정보" /></h3>
	</div>
	<!-- //Title Area E -->
	...
	<div id="contents">
		...
		<div class="pmbox mar_b15">
  	            <div class="pad_bl40">
      	        	<img src="<c:url value='/${ThreadSession.serviceLocale.language}/images/uc/uc_txt02.gif' />" alt="단말기" class="pad_tr6 fltl" />
          	        <div class="fltl pad_tr2">
					<select id="searchType" name="phoneInfo.searchType" class="w186">
					   <option value="makeComp" <c:if test="${phoneInfo.searchType eq 'makeComp' }"> selected </c:if> >제조사</option>
					   <option value="modelNm" <c:if test="${phoneInfo.searchType eq 'modelNm' }"> selected </c:if> >모델명</option>
					</select>
				</div>
				...
				<div class="fltl pad_tr2" id="searchInput">
					<input type="text" id="searchText" name="phoneInfo.searchText" value="<g:html value='${phoneInfo.searchText }'/>" style="width: 256px" />
				</div>					
		...
		<div class="sdinfo">
			<table summary="단말정보 목록">
				<caption>단말정보 목록</caption>
				<colgroup>
					<col width="14%" />
					<col />
				</colgroup>
				<tbody>
					<c:forEach items="${resultList }" var="phoneInfo" varStatus="status">
							...
							<td rowspan="2" class="tit01">
								...
								<c:choose>
									<c:when test="${not empty phoneInfo.listImgPos }">
										<img src="${CONF['omp.common.url.http-share.phone']}/${phoneInfo.listImgPos}" width="72" height="72" onerror="javascript:fixBroken(this, '<c:url value="/${ThreadSession.serviceLocale.language}/images/uc/no_images.gif"/>');" alt="상품 이미지" />
									</c:when>
									<c:otherwise>
						...
						<tr>
							<td>
								<ul class="sdinfo">
									<li><img src="<c:url value='/${ThreadSession.serviceLocale.language}/images/uc/uc_txt03.gif' />" alt="제조사" />&nbsp;&nbsp; <gc:text code="${phoneInfo.makeComp }"/> </li>
									<li><img src="<c:url value='/${ThreadSession.serviceLocale.language}/images/uc/uc_txt05.gif' />" alt="단말매칭" />&nbsp;&nbsp; ${phoneInfo.modelNm } </li>
									...
									<li><img src="<c:url value='/${ThreadSession.serviceLocale.language}/images/uc/uc_txt06.gif' />" alt="플랫폼" />&nbsp;&nbsp; <gc:text code="${phoneInfo.vmType }" /> </li>
								</ul>
							</td>
						</tr>
					</c:forEach>
