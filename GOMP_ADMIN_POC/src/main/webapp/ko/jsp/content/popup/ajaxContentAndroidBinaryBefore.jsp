...
$(document).ready(function() {
	...
	if(${resultMap.resultCode} == true 
			&& "maxSdkVersionRcmnd" == "${resultMap.errorMessage}") {
		alert("<gm:string value='jsp.content.contentAndroidDevInfo.msg.subContent25' />");
	}
});
//-->
</script>

<c:if test="${resultMap.resultCode == true}">
	<c:choose>
		...
		&nbsp;
			...
			<input type="hidden" name="subContent.delYn" value="${subContent.delYn}"  id = "delYn${resultMap.subContentsCnt}"/>
				...
				<tbody>
					...
					<tr>
						...
						<td>
							...
							<a href="javascript:removeTempRunFile('editForm${resultMap.subContentsCnt}', '${resultMap.subContentsCnt}');"><img src="<c:url value='/${ThreadSession.serviceLocale.language}/images/pm/btn_del02.gif'/>" alt="삭제" /></a>
						</td>
					</tr>
					<tr>
						<td class="lh160">
							minSdkVersion : ${resultMap.minSDKVersion} (${resultMap.vmVerMin})<br />
							targetSdkVersion : ${resultMap.targetSDKVersion}
							<c:if test="${not empty resultMap.vmVerTarget && resultMap.vmVerTarget ne 'N/A'}">(${resultMap.vmVerTarget})</c:if><br />
							maxSdkVersion : ${resultMap.maxSDKVersion}
							<c:if test="${not empty resultMap.vmVerMax}">(${resultMap.vmVerMax})</c:if><br />
...
<c:if test="${resultMap.resultCode == false && resultMap.errorMessage != null}">
	<c:choose>
		...
		<c:when test="${resultMap.errorMessage == 'DebugModeTrueError'}">
			<gm:string value='jsp.content.contentAndroidDevInfo.msg.subContent13' />
		</c:when>
		...
		<c:when test="${resultMap.errorMessage == 'NoSamePackageName'}">
			<gm:string value='jsp.content.contentAndroidDevInfo.msg.subContent17' />
		</c:when>
		<c:when test="${resultMap.errorMessage == 'NotVersionCodeError'}">
			<gm:string value='jsp.content.contentAndroidDevInfo.msg.subContent24' />
		</c:when>
		<c:when test="${resultMap.errorMessage == 'WrongTargetSDKVersion1'}">
			<gm:string value='jsp.content.contentAndroidDevInfo.msg.subContent19' />
		</c:when>
		<c:when test="${resultMap.errorMessage == 'WrongTargetSDKVersion2'}">
			<gm:string value='jsp.content.contentAndroidDevInfo.msg.subContent19' />
		</c:when>
		<c:when test="${resultMap.errorMessage == 'SpecNoneMinSdkVersion'}">
			<gm:string value='jsp.content.contentAndroidDevInfo.msg.subContent20' />
		</c:when>
		<c:when test="${resultMap.errorMessage == 'SpecNoneMaxSdkVersion'}">
			<gm:string value='jsp.content.contentAndroidDevInfo.msg.subContent21' />
		</c:when>
		<c:when test="${resultMap.errorMessage == 'SpecNoneTargetSdkVersion'}">
			<gm:string value='jsp.content.contentAndroidDevInfo.msg.subContent22' />
		</c:when>
		<c:when test="${resultMap.errorMessage == 'NoSamePrivateSignKey'}">
			<gm:string value='jsp.content.contentAndroidDevInfo.msg.subContent23' />
		</c:when>
		<c:when test="${resultMap.errorMessage == 'NoneVersionName'}">
			<gm:string value='jsp.content.contentAndroidDevInfo.msg.subContent26' />
		</c:when>
		<c:otherwise>
