...
function setAdjAmt(){
	...
	if(showValidate('popUpForm', 'default', "<gm:string value=""/>")){
		if (adjAmt.length==1){ //조정액이 한자리 숫자일때
		...
		$('#accountsSS\\.adjAmtEditYN', opener.document).val("Y");
		opener.setAdjAmt();
		self.close();	
	}
...
<body class="popup" > 
	<div id="popup_area_440">
    	...
    	<table class="pop_tabletype01">
        	...
        	<tbody>
            	<tr>
                	<th>분류</th>
                    <td>
                    	<input type="radio" checked="checked" id="accountsSS.pmCheck" name="accountsSS.pmCheck" value=""   /> +
                    	<input type="radio" class="ml10" id="accountsSS.pmCheck" name="accountsSS.pmCheck" value="-"   /> -
                    </td>
                </tr>
                <tr>
                	<th>금액</th>
                    <td>
                    	<input type="text" class="txt" style="width:280px" id="accountsSS.adjAmt" onKeyPress="preventSubmit()" onKeyUp = "js_OnlyNumberR(this);" 
                    		v:required m:required="<gm:string value='jsp.settlement.accounts.popUpAdjAmt.changemoney'/>"
                    	/>
                    </td>
                </tr>
                <tr>
		...
		<div class="pop_btn" >
			<a class="btn_s" href="#" id="confirmBtn" onClick="JavaScript:setAdjAmt()"><strong id="confirmBtn">확인</strong></a>
