...
<body>
	...
	<table class="tabletype01">
		...
		<tr>
			<th>발급기준</th>
			<td class="align_td">
				<s:radio id="issueType" name="sub.issueType" list="#{'all':'전체','specific':'대상 지정 발급','excelRegi':'엑셀 등록 발급'}" value="sub.issueType" required="true" tooltip="<gm:string value='jsp.gift.presentCashList.msg.tooltip'/>"/>
			</td>
		</tr>
	</table>
	</s:form>
	<p class="fl mt35">총 <strong class="point2">${totalCount }</strong>건이 검색되었습니다.</p>			
	<p class="fr mt25 mb05">
		<a class="btn" href="#" id="reqCashBtn"><span>Cash 발급하기</span></a> 
	...
	<table class="tabletype02">
		...
		<tr>
			...
			<td><c:choose><c:when test="${giftCard.cardType eq 'OR002905' }">${giftCard.cardCount} 명</c:when><c:otherwise>${giftCard.cardCount }</c:otherwise></c:choose></td>
