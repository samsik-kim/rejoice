...
<script language='javascript'>
	...
	function funcUpdateAdminRecommend() {
		...
		if(chkLen > 0){
			...
			if(!confirm("<gm:string value='jsp.contents.adminrec.adminBeginner.msg.update_confirm'/>")){
				return;
			}						
	...
	function funcDeleteAdminRecommend() {
		if ( $( "input:checkbox[name=checkProdId]:checked" ).length == 0 )	{
			alert( "<gm:string value='jsp.contents.adminrec.adminBeginner.msg.delete_check'/>" );
			return;
		}

		var selectedProdId =  makeProdIdStr();
		
		if(!confirm("<gm:string value='jsp.contents.adminrec.adminBeginner.msg.delete_confirm'/>")){
			return;
		}	
...
*/
	...
	function funcDecCategoryPrior(idx) {
		...
		if((Number(num) - 1) < 1) {
			alert("<gm:string value='jsp.contents.adminrec.adminBeginner.msg.category_prior'/>");
			document.getElementsByName("expoPrior")[idx].value = num;
			return;
		}
	...
	$(document).ready( function()	{
		...
		$( "#checkAll" ).click( function()	{
			...
			<div id="location">
				Home &gt; 컨텐츠관리 &gt; 초보자용관리 &gt; <strong>초보자용관리</strong> 
			</div><!-- //location -->
			<!-- 2011-03-18 -->
			<div class="overflow_h">
				<h1 class="fl pr10">초보자용관리</h1>
				<p>운영자는 초보자용 컨텐츠를 관리 할수 있다.</p>
			</div>	
			...
			<p class="fl mt20 mb05"><a class="btn_s" href="javascript:funcPopProd();"><span>등록</span></a></p>
			...
			<p class="fl mt05"><a class="btn_s" href="javascript:funcDeleteAdminRecommend();"><span>선택삭제</span></a></p>
			<p class="btn_wrap text_r mt05"><a class="btn" href="javascript:funcUpdateAdminRecommend();"><span>일괄변경</span></a></p>
			<!-- paging -->
