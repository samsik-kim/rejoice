...
$(document).ready(function(){
	...
	if('${mobileBanner.bnrNo}' == "") {
		...
		document.bannerListForm.aid.value = '${mobileBanner.prodId }';
...
<s:form id="bannerListForm" name="bannerListForm" theme="simple" method="post" enctype="multipart/form-data">
	...
	<table class="tabletype01">
		...
		<tr><!-- write – URL 링크 등록 -->
			...
			<td class="align_td">	
				...
				<a class="btn_s" href="javascript:funcPopProd();"><span>검색</span></a>
                </span>
		...
		<tr>
			...
			- 이미지 사이즈 000X pixel 타입 : PNG 
			</td>
		</tr>
		<tr>
			<th>노출설정</th>
			<td class="align_td">
				<p class="fr">*노출순서를 입력하지 않을경우 첫번째 배너로 노출 설정됩니다.</p>					
				<input type="radio" id="openY" name="mobileBanner.openYn" class="ml05" value="Y" <c:if test="${mobileBanner.openYn eq 'Y' || mobileBanner.openYn eq '' }">checked</c:if> />노출  &nbsp;
				<input type="radio" id="openN" name="mobileBanner.openYn" class="ml05" value="N" <c:if test="${mobileBanner.openYn eq 'N' }">checked</c:if> />숨김 
				<!--<span id="open" name="open" > -->
				<label for="#" class="ml30">노출순서</label>
				<input id="openOrder" name="mobileBanner.openOrder" type="text" class="txt" style="width:30px;" v:regexp="[1-9][0-9]*" m:regexp="<gm:tagAttb value='jsp.bnrMg.mblBnrList.msg.orderchk'/>" value="${mobileBanner.openOrder }" />번째 
		...
		<tr>
			<th>기간지정<input type="checkbox" id="term" name="term" checked/></th>
			<td class="align_td">				
	...
	<c:if test="${mobileBanner.bnrNo ne '' }">
		<a class="btn" href="javascript:deleteBanner('${mobileBanner.bnrNo}');"><span>삭제</span></a>
	</c:if>
		<a class="btn" href="javascript:saveBanner();"><span>저장</span></a>
		<a class="btn" href="javascript:go_List();"><span>목록</span></a>
	</p>
</s:form>
</body>
</html>