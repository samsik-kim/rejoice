...
<s:form id="ctgrQnaForm" name="ctgrQnaForm" theme="simple" method="post" enctype="multipart/form-data">
	...
	<c:if test="${url eq 'registerDCtgr.omp' || url eq 'deleteDCtgrQnA.omp' || url eq 'saveDCtgrQnA.omp'}">
		首頁  &gt; 客戶服務中心 &gt; 客戶諮詢管理 &gt; <strong>開發商諮詢類別管理</strong>
	</c:if>
	<c:if test="${url eq 'registerCtgr.omp' || url eq 'deleteCtgrQnA.omp' || url eq 'saveCtgrQnA.omp'}">
		首頁  &gt; 客戶服務中心 &gt; 客戶諮詢管理 &gt; <strong>SC客戶諮詢類別管理</strong> 
	</c:if>
	</div>
	<!-- //location -->
	<div class="overflow_h" style="">
		<h1 class="fl pr10"><c:if test="${url eq 'registerDCtgr.omp' || url eq 'deleteDCtgrQnA.omp' || url eq 'saveDCtgrQnA.omp'}">開發商諮詢類別管理</c:if><c:if test="${url eq 'registerCtgr.omp' || url eq 'deleteCtgrQnA.omp' || url eq 'saveCtgrQnA.omp'}">SC客戶諮詢類別管理</c:if></h1>
		<p><c:if test="${url eq 'registerDCtgr.omp' || url eq 'deleteDCtgrQnA.omp' || url eq 'saveDCtgrQnA.omp'}">可新增／變更／刪除開發商諮詢類別。</c:if><c:if test="${url eq 'registerCtgr.omp' || url eq 'deleteCtgrQnA.omp' || url eq 'saveCtgrQnA.omp'}">可新增／變更／刪除SC客戶諮詢類別。</c:if></p>
	</div>
	<div id="adm_cate">
		<dl class="mr15">
			<dt>大類別</dt>
			...
			<dd>
				...
				<p class="fr">
					<a class="btn_s" href="javascript:modifyNm01();"><span>變更</span></a>
					<a class="btn_s" href="javascript:deleteCtgr01();"><span>刪除</span></a>
				</p>
			</dd>
			<dd class="mt05">
				<input id="ctgr01text" type="text" class="txt" style="width:88%;" value="<gm:html value=""/>" v:required="trim" m:required="<gm:tagAttb value='jsp.community.qna.qna_ctgr.text.ctgrchk'/>" v:text8_limit="50" m:text8_limit="<gm:tagAttb value='jsp.community.qna.qna_ctgr.msg.50'/>"/><a class="btn_s" href="javascript:insertCtgr01();"><span>新增</span></a>
				<p class="btn_wrap text_r mt25"><a class="btn" href="javascript:saveCtgr01();"><span>儲存</span></a></p>
			</dd>
		</dl>
		<c:if test="${url eq 'registerCtgr.omp' || url eq 'deleteCtgrQnA.omp' || url eq 'saveCtgrQnA.omp'}">
		<dl>
			...
			<dd>
				...
				<p class="fr">
					<a class="btn_s" href="javascript:modifyNm02();"><span>變更</span></a>
					<a class="btn_s" href="javascript:deleteCtgr02();"><span>刪除</span></a>
				</p>
			</dd>
			<dd class="mt05">
				...
				<p class="btn_wrap text_r mt25">
					<a class="btn" href="javascript:saveCtgr02();"><span>儲存</span></a>
				</p>
			</dd>
		</dl>
		</c:if>
