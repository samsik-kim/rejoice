...
<s:form id="detailQnaForm" name="detailQnaForm" theme="simple" method="post" enctype="multipart/form-data">
	...
	<!-- //location -->
	<h1 class="fl pr10"><c:if test="${url eq 'displayDQnA.omp' || url eq 'sendDMail.omp'}">開發商諮詢</c:if><c:if test="${url eq 'displayQnA.omp' || url eq 'sendMail.omp'}">SC客戶諮詢</c:if></h1>
	<c:if test="${url eq 'displayDQnA.omp' || url eq 'sendDMail.omp'}"><p>可查看／答覆開發商諮詢。</p></c:if><c:if test="${url eq 'displayQnA.omp' || url eq 'sendMail.omp'}"><p>可查看/回復客戶諮詢。</p></c:if>
	<table class="tabletype02">
		...
		<tbody>
			<c:forEach items="${questionList}" var="item" varStatus="cnt">
				<tr>
				<th <c:if test="${url eq 'displayDQnA.omp' || url eq 'sendDMail.omp'}">colspan="2"</c:if>>狀態</th><!-- 조치상태  -->
				...
				<tr>
					<th <c:if test="${url eq 'displayDQnA.omp' || url eq 'sendDMail.omp'}">colspan="2"</c:if>>諮詢者</th>
					<td class="text_l">
				...
				<tr>
					<th <c:if test="${url eq 'displayDQnA.omp' || url eq 'sendDMail.omp'}">colspan="2"</c:if>>電子郵件</th>
					<td colspan="3" class="text_l">
				...
				<tr>
					<th <c:if test="${url eq 'displayDQnA.omp' || url eq 'sendDMail.omp'}">colspan="2"</c:if>>標題</th>
					<td colspan="3" class="text_l align_td">
						<s:select id="qnaCtgr01" name="qna.ctgrCd" theme="simple" list="categoryNameList" listKey="ctgrCd" listValue="ctgrNm" value="%{qnaClsCd}" headerKey="" headerValue="全部 " style="width:104px;"/>
						<!-- 2차카테고리 -->
						<c:if test="${url eq 'displayQnA.omp' || url eq 'sendMail.omp'}">
						...
						<s:else>
							<s:select id="qnaCtgr02" name="qna.qnaClsCd2" theme="simple" list="secondDepthCategoryList" listKey="ctgrCd" listValue="ctgrNm" value="%{qnaClsCd2}" headerKey="" headerValue="全部 " style="width:104px;"/>
						</s:else>
						</c:if>
						<c:if test="${url eq 'displayDQnA.omp' || url eq 'sendDMail.omp'}">
				...
				<tr>
					<th <c:if test="${url eq 'displayDQnA.omp' || url eq 'sendDMail.omp'}">colspan="2"</c:if>>內容</th>
					<td colspan="3" class="text_l">
						...
						<c:choose>
							...
							<c:otherwise>
							<tr>
								<th <c:if test="${url eq 'displayDQnA.omp' || url eq 'sendDMail.omp'}">colspan="2"</c:if>>答覆</th>
								<td colspan="3" class="text_l">
									無答覆。
								</td>
							</tr>
	...
	<c:if test="${url eq 'displayDQnA.omp' || url eq 'sendDMail.omp'||((url eq 'displayQnA.omp' || url eq 'sendMail.omp')&&prcstcd eq 'CM000701')}">
		<table class="tabletype02 mt05">
			...
			<thead>
				<tr>
					<th colspan="2">填寫答覆後點選［發送］，答覆將發送至會員之電子郵件。</th>
				</tr>
			</thead>
			<tbody>
				<tr>
		...
		<c:if test="${url eq 'displayDQnA.omp' || url eq 'sendDMail.omp'||((url eq 'displayQnA.omp' || url eq 'sendMail.omp')&&prcstcd eq 'CM000701')}">
		<a class="btn" href="javascript:func_registryQnA();"><span>發送</span></a> 
		</c:if>
		<a class="btn" href="javascript:go_List();"><span>目錄</span></a></p>
</s:form>
</body>