...
<body class="popup">
	<div id="popup_area_440">
		...
		<c:if test="${empty param.dpCatNo}">
          <p class="mt10">※ 上傳IMG</p>
		</c:if>
		<div class="pop_btn" >
			<p>
		<c:if test="${empty param.dpCatNo}">
			<a class="btn_s" href="javascript:funcInsertCategory();"><strong>儲存</strong></a>
		</c:if>
		<c:if test="${not empty param.dpCatNo}">
			<a class="btn_s" href="javascript:funcUpdateCategory();"><strong>變更</strong></a>
		</c:if>
			<a class="btn_s" href="javascript:self.close();"><strong>取消</strong></a></p>
		</div>
		</s:form>
	</div>
</body>
