...
<script language='javascript'>
	...
	function funcSearch(){
		...
		document.searchForm.action="selectNewUpdateList.omp";
	...
	function funcUpdateAdminRecommend() {
		...
		if(chkLen > 0){
			...
			if(!confirm("<gm:string value='jsp.contents.newUpdate.msg.update_confirm'/>")){
				return;
			}						
			...
			document.searchForm.action = './updateNewUpdateList.omp';
			document.searchForm.submit();
			}
		} else {
			alert("<gm:string value='jsp.contents.newUpdate.msg.update_check'/>");
			return;
		}
	...
	function funcDeleteAdminRecommend() {
		if ( $( "input:checkbox[name=checkProdId]:checked" ).length == 0 )	{
			alert( "<gm:string value='jsp.contents.newUpdate.msg.delete_check'/>" );
			return;
		}

		var selectedProdId =  makeProdIdStr();
		
		if(!confirm("<gm:string value='jsp.contents.newUpdate.msg.delete_confirm'/>")){
			return;
		}	
		
		document.searchForm.action = './deleteNewUpdateList.omp?selectedProdId='+selectedProdId;
		document.searchForm.submit();
	};
/*
...
*/
	...
	function funcDecCategoryPrior(idx) {
		...
		if((Number(num) - 1) < 1) {
			alert("<gm:string value='jsp.contents.newUpdate.msg.category_prior'/>");
			document.getElementsByName("expoPrior")[idx].value = num;
			return;
		}
	...
	$(document).ready( function()	{
		...
		$( "#checkAll" ).click( function()	{
			...
			<div id="location">
				首頁 &gt; 內容管理中心 &gt; 最新產品管理 &gt; <strong>最新產品管理</strong> 
			</div><!-- //location -->

			<h1 class="fl pr10">最新產品管理</h1>
			<p>可管理最新產品之公開與否和排列順序</p>
			...
			<table class="tabletype02">
				...
				<tr>
					...
					<td><g:html value="${adminRecommendList.regDt}" format="L####-##-##" /></td>
					<td class="align_td">
