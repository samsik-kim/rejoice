...
$(document).ready(function(){
	...
	$("#loginBtn").click(function(){
		if(showValidate('frm', 'default', "<gm:string value='jsp.common.msg.title01'/>")){
			saveid();
			$('#loginFrm').attr('action', "${CONF['omp.common.url-prefix.https.dev']}" + env.contextPath + '/login/login.omp');
			$('#loginFrm').submit();
			return false;
		}
	} );
...
function gotoContentsList(objName) {
	...
	<div id="main_container">
		<h2 class="hide">Main Contents Area</h2>
		...
		<div id="maincontent">
			...
			<div id="mcleft">   
				...
				<c:if test="${empty MEMBER_SESSION}">
					<div class="mlogin">
						...
						<ul>
							...
							<li class="txt01"><a href="javascript:gotoPage('CHECKBEFOREREGIST');"><span class="txtcolor03 txt_bold">註冊會員</span></a></li>
							<li class="txt02"><a href="${CONF['omp.common.url-prefix.https.dev']}${pageContext.request.contextPath }/login/findId.omp">查詢帳號/密碼</a></li>
						</ul>
						</form>
					</div>   
				</c:if>
				<c:if test="${not empty MEMBER_SESSION}">
					...
					<input type="hidden" id="saleSearchType" name="content.saleSearchType" value="">
						<p class="member">歡迎<strong> <span class="txtcolor03 font_en"><g:html  value='${MEMBER_SESSION.mbrId}' /></span>會員</strong> <br /><gc:html code='${member.mbrClsCd}' />(<g:text format="L##"><gc:html code='${member.mbrCatCd}' /></g:text>)</p>
						<ul class="state">
							<li class="sta01"><span>新上架產品</span>&nbsp;<a href="javascript:gotoContentsList('CONTENT_SALE_STAT_NA');" class="font_en"><g:text value="${newProcuctCnt}" format=",###" /></a> 件</li>
							<li class="sta02"><span>已審核產品</span>&nbsp;<a href="<c:url value='/content/getContentsVerifyList.omp?ctVerify.searchVerifyPrgrYn=${CONST.CODE_VERIFY_END}'/>" class="font_en"><g:text value="${authDoneProductCnt}"  format=",###"/></a> 件</li>
							<li class="sta03"><span>昨日販售情況</span>&nbsp;<a href="<c:url value='/settlement/dailysale/dailySaleList.omp'/>" class="font_en"><g:text value="${beforeSaleCnt}" format=",###"/></a> 件</li>
							<li class="sta04"><span>昨日販售金額</span>&nbsp;<a href="<c:url value='/settlement/dailysale/dailySaleList.omp'/>" class="font_en">NT$ <g:text value="${beforeSaleSum}" format=",###"/></a></li>
						</ul>
						<ul class="proli">
							<li><a href="<c:url value='/content/registContentWrite.omp' />"><img src="<c:url value='/${ThreadSession.serviceLocale.language}/images/main/proli_01.gif' />" alt="產品上架" class="vm" /></a></li>
							<li><a href="<c:url value='/content/getContentsVerifyList.omp' />"><img src="<c:url value='/${ThreadSession.serviceLocale.language}/images/main/proli_02.gif' />" alt="審核現狀" class="vm" /></a></li>
							<li><a href="<c:url value='/settlement/dailysale/dailySaleList.omp'/>"><img src="<c:url value='/${ThreadSession.serviceLocale.language}/images/main/proli_03.gif' />" alt="結算現狀" class="vm" /></a></li>
						</ul>
					</form>	
					</div>
				</c:if>
				<!-- //Login E-->
				<c:if test="${empty MEMBER_SESSION}">
					<p class="cb"><img src="<c:url value='/${ThreadSession.serviceLocale.language}/images/main/open_banner3.gif' />" alt="Whoopy Developer Center Grand Open" /></p>
				</c:if>
				<div class="mdown">     
			...
			<div id="mcright">
				<div class="guide1">
					...
					<p class="gtxt"><img src="<c:url value='/${ThreadSession.serviceLocale.language}/images/main/gtxt_01.gif' />" alt="可販售自行開發之App..可在此方便快捷地進行 .販售及管理." /></p>
				...
				<div class="guide2">
					...
					<p class="gtxt"><img src="<c:url value='/${ThreadSession.serviceLocale.language}/images/main/gtxt_02.gif' />" alt="為了App之服務順暢,將提供有害性及末端啟 動機制服務." /></p>
				...
				<div class="guide3">
					...
					<p class="gtxt"><img src="<c:url value='/${ThreadSession.serviceLocale.language}/images/main/gtxt_03.gif' />" alt="可確認產品販售狀況, 還將為您提供每月結算 報告." /></p>
				...
				<div class="devnews">
					<div class="tit">
						...
						<p class="txtr pad_t5"><a href="<c:url value='/community/listNotice.omp' />"><img src="<c:url value='/${ThreadSession.serviceLocale.language}/images/main/more.gif' />" alt="Developer Center News 更多" /></a></p>
					</div>
					<form id="noticeForm" name="noticeForm" method="get" action="">
						...
						<dl>
							<dt>
								<a href="javascript:noticeView('${notice.noticeId}');"><g:html value="${notice.title}" /></a>
								<c:if test="${notice.newYn eq 'Y' }">
									&nbsp;<img src="<c:url value='/${ThreadSession.serviceLocale.language}/images/main/icon_new.gif' />" alt="new" class="vm pad_b2" />
								</c:if>
								...
								&nbsp;<a href="<c:url value="/fileSupport/bbsFileDown.omp">
															...
															</c:url>" ><img src="<c:url value='/${ThreadSession.serviceLocale.language}/images/main/icon_addfile.gif' />" alt="addfile" class="vm pad_b2" /></a>
								</c:if>
