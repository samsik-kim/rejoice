	...
	<div id="contents">
		...
		<div class="tstyleD mar_b30">
			<table summary="審核現狀">
				...
				<tbody>
					<tr>
						<th scope="row"><img src="<c:url value='/${ThreadSession.serviceLocale.language}/images/pm/pm_txt07.gif' />" alt="請審日期" /></th>
						<td class="tit02">
							<input type="text" id="startDate" name="ctVerify.startDate" class="w109" v:required m:required="<gm:string value='jsp.content.verify.contentsVerifyList.msg.dateChk02'/>" m:scompare="<gm:string value='jsp.content.verify.contentsVerifyList.msg.dateChk03'/>"
								value="<g:text value='${ctVerify.startDate }' format='L####-##-##' />" readonly="readonly" title="start" /> &nbsp;~&nbsp;
							<input type="text" id="endDate" name="ctVerify.endDate" class="w109" v:required m:required="<gm:string value='jsp.content.verify.contentsVerifyList.msg.dateChk04'/>"
								...
								value="<g:text value='${ctVerify.endDate }' format='L####-##-##' />" readonly="readonly" title="end" />
							&nbsp;&nbsp; 最近
							<a href="#"><img id="today" name="today" src="<c:url value='/${ThreadSession.serviceLocale.language}/images/pm/btn_1day.gif' />" alt="1日" onclick="javascript:payDateSet('today');" /></a>
							<a href="#"><img id="7day" name="7day" src="<c:url value='/${ThreadSession.serviceLocale.language}/images/pm/btn_7day.gif' />" alt="7日" onclick="javascript:payDateSet('7day');" /></a>
							<a href="#"><img id="1month" name="1month" src="<c:url value='/${ThreadSession.serviceLocale.language}/images/pm/btn_1mon_on.gif' />" alt="1個月" onclick="javascript:payDateSet('1month');" /></a>
							<a href="#"><img id="3month" name="3month" src="<c:url value='/${ThreadSession.serviceLocale.language}/images/pm/btn_3mon.gif' />" alt="3個月" onclick="javascript:payDateSet('3month');" /></a>

						</td>
					</tr>
					<tr>
						<th scope="row"><img src="<c:url value='/${ThreadSession.serviceLocale.language}/images/pm/pm_txt08.gif' />" alt="平台" /></th>
						<td class="tit02">
					...
					<tr>
						<th scope="row"><img src="<c:url value='/${ThreadSession.serviceLocale.language}/images/pm/pm_txt09.gif' />" alt="審核狀態" /></th>
						...
						<td>
							<c:forEach items="${verifyPrgrYnCodeList }" var="verifyPrgrYnCode">
								<input type="checkbox" id="searchVerifyPrgrYn" name="ctVerify.searchVerifyPrgrYn" value="${verifyPrgrYnCode.addField2 }"
								<c:forEach items="${ctVerify.searchVerifyPrgrYn }" var="verifyPrgrYn">
									<c:if test="${verifyPrgrYnCode.addField2 ==  verifyPrgrYn}"> checked </c:if>
								</c:forEach>
								> ${verifyPrgrYnCode.addField1} </input> &nbsp;&nbsp;
							</c:forEach>
						</td>
					</tr>
					<tr>
						<th scope="row"><img src="<c:url value='/${ThreadSession.serviceLocale.language}/images/pm/pm_txt10.gif' />" alt="審核結果" /></th>
						<td class="tit02">
					...
					<tr>
						<th scope="row" class="tit01"><img src="<c:url value='/${ThreadSession.serviceLocale.language}/images/pm/pm_txt11.gif' />" alt="產品名稱" /></th>
						<td class="bgnone">
							<input type="text" id="prodNm" name="ctVerify.prodNm" class="w410" value="<g:tagAttb value="${ctVerify.prodNm }"/>" title="搜尋" />
							<img id="searchBtn" name="searchBtn" src="<c:url value='/${ThreadSession.serviceLocale.language}/images/pm/btn_search.gif' />" style="cursor: pointer;" alt="搜尋" />
						</td>
					</tr>
		...
		<p class="pbult09">共有 <span class="txtcolor02">${ctVerify.page.totalCount }</span>&nbsp;個產品.</p>
		<div class="btstyleA">
			<table summary="審核現狀">
				<caption>審核現狀</caption>
				...
				<thead>
					<tr>
						...
						<th scope="col"><img src="<c:url value='/${ThreadSession.serviceLocale.language}/images/pm/th_tit26.gif' />" alt="平台" /></th>
						<th scope="col"><img src="<c:url value='/${ThreadSession.serviceLocale.language}/images/pm/th_tit38.gif' />" alt="產品名稱" /></th>
						<th scope="col"><img src="<c:url value='/${ThreadSession.serviceLocale.language}/images/pm/th_tit27.gif' />" alt="請審日期" /></th>
								...
								<tr>
									...
									<td class="tit01">
										<span class="imgbox">
											<c:choose>
												...
												<c:otherwise>
													<img src="<c:url value='/${ThreadSession.serviceLocale.language}/images/pm/no_images.gif' />" alt="photo" />
												</c:otherwise>
											</c:choose>
										</span>
									...
									<td>
										<c:if test="${content.verifyPrgrYn eq CONST.CODE_VERIFY_ING || content.verifyPrgrYn eq CONST.CODE_VERIFY_TEST_END || content.verifyPrgrYn eq CONST.CODE_VERIFY_TEST_REJECT}">
											<c:if test="${0 > content.ctEndExDt}"><div align="center"> D${content.ctEndExDt } 日</div></c:if>
											<c:if test="${0 < content.ctEndExDt}"><div align="center"> D+${content.ctEndExDt } 日</div></c:if>
											<c:if test="${0 == content.ctEndExDt}"><div align="center"> D-${content.ctEndExDt } 日</div></c:if>
