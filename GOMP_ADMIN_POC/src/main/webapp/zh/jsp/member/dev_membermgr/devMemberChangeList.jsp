...
<div id="location">
	首頁 &gt; 會員管理中心 &gt; 開發商管理 &gt; <strong>轉換會員記錄</strong>
</div><!-- //location -->
...
<tr>
	...
	<td class="align_td line2_5">
		...
		<label for="#" style="width:60px;">轉換申請日期</label>
...
<tr>
	...
	<td class="align_td">
	    <select id="searchType" name="sc.searchType" style="width:104px;">
			...
			<option value="name" <c:if test="${sc.searchType eq 'name' }">selected</c:if>>姓名/公司名稱</option>
	    </select>
	...
	<tbody>
		<c:choose>
			<c:when test="${searchYn eq 'N'}">
				<tr><td colspan="9"><gm:string value="jsp.admin.common.first.page"/></td></tr>
			</c:when>
			<c:otherwise>
				<c:choose>	
