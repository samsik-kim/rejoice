...
public class ContentDetailInfoServiceImpl 
	...
	public List<Category> getCategoryList() {
		...
		try{
			String upDpCatNo = "DP01";		// 1Depth Category
	...
	public String modifyContentDetail(Contents content, Contents oldContent) {
		...
		int limitPrice = conf.getInt("omp.dev.product.contents.prodPrc.limit", 2000);
		
		if (Integer.parseInt(content.getProdPrc()) > limitPrice) {
			log.debug("Prod Price Limit : {0}, {1}", new Object[] {Integer.parseInt(content.getProdPrc()), limitPrice});
			result = "PROD_PRC_LIMIT";
			return result;
		}
		...
		try {	
			...
			// IMAGES MODIFY & SAVE
			result = modifyContentImages(content);
			
			if ("THUMBNAIL_UPDATE_FAIL".equals(result)) {
				return result;
			} else if ("IMAGE_UPDATE_FAIL".equals(result)) {
				return result;
			}
	...
	private void setContentBaseInfoValue(Contents content) {
		...
		try {
			...
			content.setProdDescSmmr(CommonUtil.replaceAllTagsExcept(content.getProdDescSmmr(),""));			
			content.setProdDescDtl(CommonUtil.replaceAllTagsExceptA(content.getProdDescDtl(),""));
			content.setCorpProdNo(CommonUtil.removeSpecial(content.getCorpProdNo()));
	 ...
	 * @see com.omp.dev.contents.service.ContentDetailInfoService#getContentTagNameList(java.lang.String)
	 */
	@SuppressWarnings("unchecked")
	...
	private ContentImage insertContentImage(String imgType, ContentImage contentImage, String pathPreFix, String filePath, String cid) {
		...
		try {
			...
			if (log.isDebugEnabled()) {
				log.debug("ORIGINAL Info - Imb Gbn   			:: {0}", new Object[] {imgType});
				log.debug("ORIGINAL Info - File Src Pos  		:: {0}", new Object[] {contentImage.getUpload().getPath()});
				log.debug("ORIGINAL Info - File Original Name  	:: {0}", new Object[] {contentImage.getUploadFileName()});
				log.debug("ORIGINAL Info - File Pos  			:: {0}", new Object[] {imgFileFullPath.toString()});
				log.debug("ORIGINAL Info - File Nm   			:: {0}", new Object[] {imgFileName});
				log.debug("ORIGINAL Info - File Size 			:: {0}", new Object[] {contentImage.getUpload().length()});
			}
	...
	private void insertIconTumbnail(String imgType, ContentImage contentImage, String pathPreFix, String filePath, String cid, String imgWidth, String imgHeight) throws Exception{
		...
		if (log.isDebugEnabled()) {
			log.debug("ICON ThumbNail Info - Imb Gbn   			:: {0}", new Object[] {imgType});
			log.debug("ICON ThumbNail Info - File Src Pos  		:: {0}", new Object[] {iconOriginalImg.getPath()});
			log.debug("ICON ThumbNail Info - File Original Name :: {0}", new Object[] {contentImage.getFileNm()});
			log.debug("ICON ThumbNail Info - File Pos  			:: {0}", new Object[] {imgFileFullPath.toString()});
			log.debug("ICON ThumbNail Info - File Nm   			:: {0}", new Object[] {imgFileName});
			log.debug("ICON ThumbNail Info - File Size 			:: {0}", new Object[] {iconOriginalImg.length()});
			log.debug("ICON ThumbNail Info - File Width 		:: {0}", new Object[] {imgWidth});
			log.debug("ICON ThumbNail Info - File Height 		:: {0}", new Object[] {imgHeight});
		}
		...
		if (log.isDebugEnabled()) {
			log.debug("ICON ThumbNail Info - File Width 	:: {0}", new Object[] {imgWidthInt});
			log.debug("ICON ThumbNail Info - File Height 	:: {0}", new Object[] {imgHeightInt});
		}
