...
<h3><img src="${pageContext.request.contextPath }/${ThreadSession.serviceLocale.language}/images/common/mp_left_title.gif" alt="마이페이지" /></h3>
<ul class="fmenu">
	<li class="${leftMenuId eq 1 ? 'on':'' }"><a href="<c:url value='/mypage/mypageIntro.omp'/>">회원정보변경</a></li>
	<li class="${leftMenuId eq 2 ? 'on':'' }"><a href="<c:url value='/mypage/mypageEmail.omp'/>">이메일변경</a></li>
	<c:if test="${isTransMenuValid eq 'TRUE'}">
	<li class="${leftMenuId eq 3 ? 'on':'' }"><a href="<c:url value='/mypage/mypageMemberTransGuideView.omp'/>">회원전환</a></li>
	</c:if>
	<li class="${leftMenuId eq 4 ? 'on':'' }"><a href="<c:url value='/mypage/mypageMemberWithdraw.omp'/>">회원탈퇴</a></li>
</ul>