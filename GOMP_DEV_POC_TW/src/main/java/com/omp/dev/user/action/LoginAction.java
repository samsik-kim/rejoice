...
public class LoginAction extends BaseAction{
	...
	public String login()throws Exception{
		...
		if(session.getResultCode() == 100){
			...
			logger.debug("100 - 로그인 성공...");
			result = "loginResult";
		}else if(session.getResultCode() == 200){
			//아이디 없음
			logger.debug("200 - 아이디가 없음...");
			result = "fail";
		}else if(session.getResultCode() == 300){
			//비밀번호 불일치
			logger.debug("300 - 비밀번호 불일치...");
			result = "fail";
		}else if(session.getResultCode() == 400){
			logger.debug("400 - 로그인 불가 상태");
			logger.debug("개발자 상태 ==> " + session.getDevMbrStatCd());
			result = "fail";
			//Email ReAuth
			if(Constants.MEM_DEV_STATUS_REG_MOTION.equals(session.getDevMbrStatCd())){
				logger.debug("404 - 개발자 상태 ==> 가입 신청");
				result = "reSendEmail";
			}
	...
	public String ajaxSendTempPassword()throws Exception{
		...
		try {
			...
			mail.setSubject(LocalizeMessage.getLocalizedMessage("[Tstore 개발자센터] 임시비밀번호 발급 안내입니다."));
	...
	public void reEmailCertification() throws Exception{
		...
		try {
			...
			try {
				...
				mail.setSubject(LocalizeMessage.getLocalizedMessage("[Tstore 개발자센터] 이메일 재인증 입니다."));
	...
	public String findIdResult() throws Exception {
		...
		if(session != null && StringUtils.isNotEmpty(email)) {
			...
			try {
				...
				mail.setSubject(LocalizeMessage.getLocalizedMessage("[Tstore 개발자센터] 회원님께서 요청하신 아이디입니다."));
				mail.setRefDataId(session.getMbrNo());
				mail.setToAddr(session.getEmailAddr());
				mail.setContentData(model);
				msa = CommunicatorFactory.getMailSendAgent();
	...
	public String findPwResult() throws Exception {
		...
		if ("".equals(StringUtils.defaultString(email, ""))) {
			...
			try {
				...
				mail.setSubject(LocalizeMessage.getLocalizedMessage("[Tstore 개발자센터] 회원님께서 요청하신 비밀번호입니다."));
				mail.setRefDataId(session.getMbrNo());
				mail.setToAddr(session.getEmailAddr());
				mail.setContentData(model);
				msa = CommunicatorFactory.getMailSendAgent();
