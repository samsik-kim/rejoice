...
//<![CDATA[
	...
	$(function() {
		...
		for ( var i=curYear-10; i <=curYear+10 ; i++){
			if (i==curYear){
				$("#exRateS\\.exchangeYear").append("<option value='"+i+"' selected>"+i+"</option>");	
			}else{
				$("#exRateS\\.exchangeYear").append("<option value='"+i+"'>"+i+"</option>");
			}
		}
...
<body>
			...
			<div id="location">
				Home &gt; 정산관리 &gt; 정산환율관리 &gt;  <strong>정산환율관리</strong> 
			</div><!-- //location -->

			<h1 class="fl pr10">정산환율 관리</h1>
			<p>월별 정산 환율에 대한 내역을 확인하실 수 있습니다.</p>
			...
			<table class="tabletype01">
				...
				<tr>
					<th>판매월</th>
					<td class="align_td">					
						<select id="exRateS.exchangeYear" style="width:60px;"></select>
						<select id="exRateS.exchangeMonth" style="width:50px;">
					...
					<td rowspan="2" class="text_c" >
						<a class="btn" href="#" id="insertBtn"><strong>등록</strong></a>
					</td>
				</tr>
				<tr>
					<th>미국 USD</th>
			...
			<table class="tabletype02 mt20">
				...
				<tr>
					<td colspan="3" class="text_c">검색조건을 선택하신 후 ‘검색’ 버튼을 클릭하세요.</td>
				</tr>
				</c:when>
				<c:otherwise>
