	...
	$(function() {
		...
		$("#receiptS\\.receiptFromYear").change(function() {
			...
			for ( var i=selectedYear-10; i <=selectedYear+10 ; i++){
				$("#receiptS\\.receiptFromYear").append("<option value='"+i+"'>"+i+"</option>");
			}
...
<body>
			...
			<table class="tabletype01">
				...
				<tr>
					...
					<td class="align_td line2_5">					
						<label for="#">사용월</label>
                        <select id="receiptS.receiptFromYear" name="receiptS.receiptFromYear" style="width:60px;"></select>
						<select id="receiptS.receiptFromMonth" name="receiptS.receiptFromMonth" style="width:50px;">
					...
					<td class="text_c" >
						<a class="btn" href="#" id="searchReceiptBtn" ><strong>검색</strong></a>
					</td>
				</tr>
			</table>
			</s:form>
			<table class="tabletype02 mt20">
				...
				<thead>
				<tr>
					<th colspan="3">영수증번호</th>
					<th rowspan="2">사용기간</th>
					<th rowspan="2">사용 중인 번호</th>
					<th rowspan="2">남은 영수증 수</th>
				...
				<c:otherwise>	
					...
					<tr>
						<td colspan="8" class="text_c">검색조건을 선택하신 후 ‘검색’ 버튼을 클릭하세요.</td>
					</tr>
					</c:when>
					<c:otherwise>
					...
					<tr>
						...
						<td>
							...
							<a href="#" class="btn_s" onClick="JavaScript:editPage('${receipt.rtYearterm}','${receipt.occrNo}')"><span>수정</span></a>
							</c:when>
							<c:otherwise>
								-
							</c:otherwise>
			...
			<p class="btn_wrap text_r mt05"><a class="btn" href="#" id="insertBtn"><span>등록</span></a></p>
