	...
	$(document).ready(function() {
		...
		$("#btnMod").click(function() {
			if(confirm("<gm:string value='jsp.content.contentAndroidDevInfo.btn.save01'/>")) {
				...
				if(ajaxSubContsCnt('${content.cid}')) {
...
/////////////////////////////////////////////////////////////////////////////////////////////////////////////
	...
	function removeFormRunFile(objFrm, objIndex) {
		...
		fileHtml += "<th scope='row' class='tit01'><span>*</span> <label for='fileupload'>바이너리 파일등록</label></th>";
		...
		fileHtml += "			<a href='#'><img src=\"<c:url value='/${ThreadSession.serviceLocale.language}/images/pm/btn_attch.gif' />\" alt='첨부파일' /></a>";		
		...
		fileHtml += "\", ";
		fileHtml += 			index;
		fileHtml += ");'><img src=\"<c:url value='/${ThreadSession.serviceLocale.language}/images/pm/btn_upload.gif' />\" alt='업로드' id='addBinaryBtn' /></a>";
		fileHtml += "		</div>";
		fileHtml += "	</div>";
		fileHtml += "</td>";
		...
		<c:choose>
			<c:when test="${resultMap.subContentsCnt > 0}">
				<c:forEach items="${resultMap.subContsList}" var="subContent" varStatus="listIndex">	
					...
					<table summary="바이너리 파일정보 입력" id="binaryInfoTable${listIndex.index}" >
						...
						<tbody>
							...
							<tr id="binaryFileTr${listIndex.index}">
								...
								<td>
									...
									<a href="javascript:removeFormRunFile('editForm${listIndex.index}', '${listIndex.index}');"><img src="<c:url value='/${ThreadSession.serviceLocale.language}/images/pm/btn_del02.gif'/>" alt="삭제" /></a>
								</td>
							</tr>
							...
							<tr>
								<td colspan="2" class="tit02">
									...
									<a href="javascript:devContsPageReset('editForm${listIndex.index}', '${listIndex.index}');"><img src="<c:url value='/${ThreadSession.serviceLocale.language}/images/pm/btn_cancle.gif'/>" alt="취소" /></a>
								</td>
							</tr>
						</tbody>
					</table>
							...
							<td class="bgnone">
								<div class="fileinputs" id="uploadDiv1bin0"  style="display:${empty subContent.runFile.runFilePos ? 'box':'none' };" >
									...
									<div class="fakefile">
										...
										<a href="javascript:uploadRunFile('editForm${resultMap.subContentsCnt}', 'tempRunUpload${resultMap.subContentsCnt}', 'label${resultMap.subContentsCnt}', ${resultMap.subContentsCnt});"><img src="<c:url value='/${ThreadSession.serviceLocale.language}/images/pm/btn_upload.gif'/>" alt="업로드" id="addBinaryBtn" /></a>
