	...
	public Session getDevMemberByPw(String userId) throws Exception {
		...
		if (session != null) {
			...
			if (!"".equals(StringUtils.defaultString(session.getEmailAddr(), ""))) {
				...
				throw new NoticeException("jsp.user.UserServiceImple.NoticeException.msg.email_fail");
			}
		}
			...
			if(result > 0){
				...
				if(result > 0 ){
					...
					if(Constants.YES.equals(session.getEmailCertYn())){
						...
						this.setStep("UpdateLoginFailCntReset");
						commonDAO.update("User.updateLoginFailCnt", map);
						//[INSERT] LOGIN_HIST
						this.setStep("InsertLoginHist");
					...
					this.setStep("SearchLoginFailCnt");
					...
					if(cnt == 14){
						...
						map.put("userId", session.getMbrId());
						//[UPDATE] FailCnt
						this.setStep("UpdateLoginFailCnt");
						commonDAO.update("User.updateLoginFailCnt", map);
					}
