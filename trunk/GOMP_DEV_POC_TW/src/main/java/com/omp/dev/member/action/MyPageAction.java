...
public class MyPageAction extends BaseAction {
		...
		try {
			if(!SessionUtil.existMemberSession(this.req)) {		// session out
				...
				if("SUCCESS".equals(result)){
					//TODO Send Email
					String url = ReturnUrlGenerateUtil.rtnUrl(this.req, Constants.AUTH_EMAIL_CHANGE, email);
					...
					try {
						...
						mail.setSubject(LocalizeMessage.getLocalizedMessage("[Tstore 개발자센터] 이메일을 변경하시려면 인증해 주세요."));
	 ...
	 * modify user profile
	 */
//	public String mypageProfileModify() throws Exception {
//		Session mbr_sess = (Session) SessionUtil.getMemberSession(this.req);
//		String result = "PROFILE";		// 수정후 화면
//		try {
//			String mbrNo = mbr_sess.getMbrNo();
...
//			}
//			this.isProfile = this.req.getParameter("isProfile");
//		} catch(Exception e) {
//			throw new ServiceException("<MypageAction mypageProfileModify>", e);
//		}
//		
//		return result;
//	}
		...
		try {
			...
			if(!StringUtils.defaultString(this.forwardAction).equals("")) {
				...
				try {
					...
					mail.setTemplateId(templateId);
					mail.setRefAppId("MemberAction.registFinish");
					mail.setSubject(LocalizeMessage.getLocalizedMessage("[Tstore 개발자센터] 정산정보 승인대기 안내입니다."));
					mail.setRefDataId(mbrNo);
	...
	/**
	 * apply user calculate information
	 * 승인대기 이메일 발송
	 * @return
	 * @throws Exception
	 */
//	public String mypageCalculateInfoInsert() throws Exception {
//		//logger.debug("정산정보 승인요청 시작입니다");
//		Session mbr_sess = (Session) SessionUtil.getMemberSession(this.req);
//		Properties prop = (Properties) this.req.getSession().getServletContext().getAttribute("CONF");
//		String result = "CALCULATE";
//		try {
...
//			}
//		} catch(Exception e) {
//			throw new ServiceException("<MypageAction mypageCalculateInfoInsert>", e);
//		}
//		return result;
//	}
	
	/**
	 * ChangePaidMember ACTION 
	 * @return
	 * @throws Exception
	 */
	public void mypageChangePaidMember() throws Exception {
		Session session = (Session) SessionUtil.getMemberSession(this.req);
		String result = "";
		String resultMsg = "";

		purchase.setMbrNo(session.getMbrNo());
		//임시 
		//purchase.setRtnCd("0000");
//		logger.debug("PAY - BEFORE ################################ S");
//		logger.debug("카드번호 :: "+purchase.getCardNum());
//		logger.debug("카드유효기간년도 :: "+purchase.getCardexpy());
//		logger.debug("카드유효기간월 :: "+purchase.getCardexpm());
//		logger.debug("보안본호 (CVV@) :: " + purchase.getCardext());
//		logger.debug("카드 사용 여부 :: " + purchase.getUsPayYn());
//			logger.debug("결제금액 :: "+purchase.getPrc());
//		logger.debug("// PAY - BEFORE ################################ E");
		purchase = this.myPageService.transferPaidMember(this.purchase);
//		logger.debug("PAY - AFTER ################################ S");
//		logger.debug("RETURN CODE :: " + purchase.getRtnCd());
//		logger.debug("RETURN MESSAGE :: " + purchase.getRtnMsg());
//		logger.debug("// PAY - AFTER ################################ E");
			String num = purchase.getCardNum().substring(0, 4) + "-" + purchase.getCardNum().substring(4, 8) + "-****-****";
			try {
				String templateId = "/DEV/US_003.html";
				// 1. MemberEmailModel Set
				model = new MemberEmailModel();
				model.setMbrId(session.getMbrId());
				model.setCardInfo(num);
				model.setPayAmt(purchase.getPrc());
				model.setPayDt(DateUtil.getSysDate());
				model.setMainUrl(ReturnUrlGenerateUtil.rtnUrl(this.req));
				// 2. SendMailModel Set
				mail = new SendMailModel();
				mail.setTemplateId(templateId);
				mail.setRefAppId("MyPageAction.mypageChangePaidMember");
				mail.setSubject(LocalizeMessage.getLocalizedMessage("[Tstore 개발자센터] 연회비 결제 완료 안내입니다"));
		...
		result = purchase.getRtnCd();
		resultMsg = purchase.getRtnMsg();
		...
		try {
			...
			throw new ServiceException("<MypageAction mypageChangePaidMember>", e);
		} finally { if(writer != null) { writer.close(); } }
	}
