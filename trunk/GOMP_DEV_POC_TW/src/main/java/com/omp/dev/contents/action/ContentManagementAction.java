...
public class ContentManagementAction extends BaseAction {
	...
	public void ajaxVerifyReq() {
		...
		try {
			...
			if (SessionUtil.existMemberSession(this.req)) {
				...
				if(checkNullValue.equals("ErrorProdNm")) 				resultMessage = "상품명을 입력하세요";
				else if(checkNullValue.equals("ErrorExposureDevNm")) 	resultMessage = "판매자명을 입력하세요";
				else if(checkNullValue.equals("ErrorProdPrc")) 			resultMessage = "상품 가격을 입력하세요";
				else if(checkNullValue.equals("ErrorProdCtgr")) 		resultMessage = "분류 정보를 입력하세요";
				else if(checkNullValue.equals("ErrorGameDelibGrd")) 	resultMessage = "이용등급을 선택하세요";	
				else if (checkNullValue.equals("ErrorTagNm")) 			resultMessage = "태그 정보를 입력하세요.";
				else if (checkNullValue.equals("ErrorProdDescSmmr")) 	resultMessage = "요약설명을 입력하세요.";
				else if (checkNullValue.equals("ErrorProdDescDtl")) 	resultMessage = "상세설명을 입력하세요.";
				else if (checkNullValue.equals("ErrorIconImg1")) 		resultMessage = "대표아이콘을 등록하세요.";
				else if (checkNullValue.equals("getPreviewImg1") || checkNullValue.equals("getPreviewImg2") 
						|| checkNullValue.equals("getPreviewImg3") || checkNullValue.equals("getPreviewImg4")) 
																		resultMessage = "미리보기이미지를 입력하세요.";
				else if (checkNullValue.equals("NoneSubContents")) 		resultMessage = "바이너리 파일을 등록하세요.";
	...
	public String popItemPreview() {
		...
		if (SessionUtil.existMemberSession(this.req)) {
			...
			// 요약설명 -> 개발자명or회사명 으로 변경
			content.setExposureDevNm(memberInfo.getMbrNm());
