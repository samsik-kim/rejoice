...
$(document).ready(function(){
	...
	$('#sendEmail').click(function(){
		...
		if(showValidate('sendEmailFrm', 'default', "<gm:string value='jsp.common.msg.title01'/>")){
			$.ajax({
				url: env.contextPath + '/mypage/ajaxSendEmail.omp',
				dataType: 'json',
				type	: "POST",
	...
	<div id="contents">
		...
		<form action="" method="post" name="sendEmailFrm">
			<div class="txttype06">
				...
				<dl class="dlist02">
					<dt><label for="emaia"><gm:html value='현재 이메일 주소'/></label></dt>
					<dd><input type="text" id="emaia" name="emaia" value="<g:tagAttb value="${profileInfo.emailAddr}"/>" class="w166" /></dd>
				</dl>
				<dl class="dlist02">
					<dt><label for="emaib"><gm:html value='새 이메일 주소'/></label></dt>
					<dd><input type="text" id="emaib" name="profileInfo.emailAddr" class="w166"
						v:required='trim' m:required="jsp.member.mypage.msg.email02"
						v:email m:email="jsp.member.mypage.msg.email03"/></dd>
				</dl>
				<dl class="dlist02 mar_b30">
					...
					<dd><input type="password" id="userpw" name="usrPw" maxlength="16" class="w166"
						...
						v:required='trim' m:required="jsp.member.mypage.msg.pw02"/></dd>
				</dl>
		...
		<div class="btnarea mar_t30">
			<a href="#"><img src="<c:url value='/${ThreadSession.serviceLocale.language}/images/common/btn_email.gif'/>" alt="이메일 전송" id="sendEmail" /></a><a href="#"><img src="<c:url value='/${ThreadSession.serviceLocale.language}/images/common/btn_cancel.gif'/>" alt="취소" onclick="javascript:gotoPage('MAIN');"/></a>
		</div>
	</div>
	<!-- //CONTENT TABLE E-->
